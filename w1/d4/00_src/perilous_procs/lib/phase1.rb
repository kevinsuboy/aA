...
# whether or not at least one of the elements of
...
def exactly?(arr, num, &prc)
    ...
    arr.each { |a| count += 1 if prc.call(a)}
...
# elements of the original array are removed if they
# return true when given to the block. Solve this using Array#each.
...
# Write a method at_least? that accepts an array,
# a number (n), and a block as an arguments.
# The method should return a boolean indicating 
# whether or not at least n elements of the array return true when given to the block
...
# The method should return a boolean indicating
# whether or not all elements of the array return true when given to the block
# Solve this using Array#each.

def every?(arr, &prc)
    arr.each {|a| return false if !prc.call(a)}
    true
end
