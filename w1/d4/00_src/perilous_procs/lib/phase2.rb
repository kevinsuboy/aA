...
# containing elements of the original array that 
# either return true for both procs or return false for both procs.

def xnor_select(arr, prc1, prc2)
    new_arr = []
    arr.each { |a| new_arr.push(a) if (prc1.call(a) && prc2.call(a)) || (!prc1.call(a) && !prc2.call(a)) }
    new_arr
end
...
# Write a method multi_map that accepts an array,
# an optional number (n), and a block as arguments.
# The method should return a new array where 
# each element of the original array is repeatedly run through the block n times.
# If the number argument is not passed in, then the the elements should be run through 
# the block once.
...
# before all of the elements that return false when given to the block.
# This means that the new array has two partitions; 
# the order among elements in the same partition should be 
# the same as their relative order in the input array.

def proctition(arr, &prc)
    arr1 = []
    arr2 = []
    arr.each do |a|
        arr1.push(a) if prc.call(a)
        arr2.push(a) if !prc.call(a)
    end
    # [arr1,arr2].flatten
    arr1+arr2
end
