...
def bi_prime?(num)
    ...
    while red < num
        green = (1.0*num/red)
        # debugger
        return true if hash.has_key?(green)
        return true if red*red == num
        hash[red*1.0] = true
        begin
            red += 1
        end while !prime?(red) # find next prime
    end
    return false
end
...
def multiply(a,b)
    ...
    neg = (a<0 && b>0) || (a>0 && b<0)
    a = a.abs
    a.times{|aa| res+=multiply(nil,b.abs)}
    neg ? -res : res
end
...
def lucas_sequence(n)
    ...
    return [2] if n==1
    return [2,1] if n==2
    a = lucas_sequence(n-1)
    b = lucas_sequence(n-2)
    a+[a[-1]+b[-1]]
end
