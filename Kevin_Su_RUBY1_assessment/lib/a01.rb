...
def binary_search(arr, target)
  ...
  if target < arr[m]
    lidx = binary_search(arr[0..m],target)
  else
    ridx = binary_search(arr[m+1..-1],target)
  end
  return lidx if lidx
  return ridx+m+1 if ridx
  return nil
end
...
class String
  ...
  def shuffled_sentence_detector(other_string)
    ...
    str = other_string.split
    str.each do |word|
      hash[word] -= 1
    end
    return false if hash.values.any?{|el| el != 0}
    return true
  end
end
